...
def replay(data: dict, recording_sha256: str) -> dict:
    ...
    if mismatched or not true_all_base_only:
        decision = "TECHNICAL_EXTERNAL"
        why = (
            "recording mismatch: parsed totals disagree with the published "
            f"aggregates {mismatched} (true-composition rules all base-only: "
        ...
        why = (
            ...
            "forbid arm is clean, every true composition is base-only and all "
            "totals reconcile with the published aggregates."
        )
