...
class RuleEngine:
    ...
    def _join(
        ...
        """
        # ⚡ Bolt Optimization: Pre-parse body to extract variable states outside the hot loop
        parsed_body = []
        for s, r, o in body:
            parsed_body.append((s, r, o, _is_var(s), _is_var(r), _is_var(o)))

        def extend(depth: int, binding: dict[str, str]) -> Iterator[dict[str, str]]:
            if depth == len(parsed_body):
                yield binding
                return
            s, r, o, s_is_var, r_is_var, o_is_var = parsed_body[depth]
            s_val = binding.get(s) if s_is_var else s
            o_val = binding.get(o) if o_is_var else o
            if s_val is not None:
                # ⚡ Bolt Optimization: Avoid default empty list allocation
            ...
            for t0, t1, t2 in candidates:
                # ⚡ Bolt Optimization: Inline _unify logic directly into the hot loop
                if not s_is_var:
                    if s != t0:
                        continue
