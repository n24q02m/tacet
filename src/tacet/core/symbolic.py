...
class RuleEngine:
    ...
    def _join(
        ...
        """
        # ⚡ Bolt Optimization: Pre-compute static checks
        body_info = []
        for p0, p1, p2 in body:
            body_info.append((p0, p1, p2, _is_var(p0), _is_var(p1), _is_var(p2)))
        ...
        def extend(depth: int, binding: dict[str, str]) -> Iterator[dict[str, str]]:
            ...
            p0, p1, p2, v0, v1, v2 = body_info[depth]

            s_val = binding.get(p0) if v0 else p0
            o_val = binding.get(p2) if v2 else p2

            # ⚡ Bolt Optimization: Avoid list allocation on default get
            ...
            for t0, t1, t2 in candidates:
                # ⚡ Bolt Optimization: Inline _unify
                if not v0:
                    if p0 != t0:
                        continue
                elif p0 in binding and binding[p0] != t0:
                ...
                # ⚡ Bolt Optimization: Defer dict copy
                if v0 or v1 or v2:
                    out = binding.copy()
                    if v0:
                        out[p0] = t0
