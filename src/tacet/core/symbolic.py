...
class RuleEngine:
    ...
    def _join(
        ...
        def extend(depth: int, binding: dict[str, str]) -> Iterator[dict[str, str]]:
            ...
            # ⚡ Bolt Optimization: Avoid default list() allocations by using idx.get(key)
            ...
            for fact in candidates:
                t0, t1, t2 = fact
                # ⚡ Bolt Optimization: Inline _unify directly into the recursive loop
                if not s_is_var:
                    if s != t0:
                        continue
                ...
                # ⚡ Bolt Optimization: Defer dict.copy() until variable bindings are confirmed
                merged = binding.copy()
                if s_is_var and s not in merged:
                    merged[s] = t0
                if r_is_var and r not in merged:
                    merged[r] = t1
                if o_is_var and o not in merged:
                    merged[o] = t2
