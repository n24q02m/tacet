...
if _HAS_FASTAPI:
    S = Annotated[str, Field(max_length=256)]
    ...
    class DistillRequest(BaseModel):
        head: S
        relation: S
        answers: list[S] = Field(..., max_length=100)
        correct: bool | None = None

    class GraphIngestRequest(BaseModel):
        triples: list[tuple[S, S, S]] = Field(default_factory=list, max_length=10000)
        nodes: list[tuple[S, S]] = Field(default_factory=list, max_length=10000)  # (id, type)
