...
def _node_signature(graph: WorldGraph, node_id: str) -> dict[tuple[str, str], int]:
    ...
    # ⚡ Bolt Optimization: Iterate only over relations the node actually participates
    # in via O(Degree) access to _out and _in, instead of O(|Relations|) check of all relations.
