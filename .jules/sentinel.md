...
## 2025-02-22 - Missing Referrer-Policy Header in API
**Vulnerability:** The FastAPI application was missing the `Referrer-Policy` header in its HTTP middleware, potentially allowing the `Referer` header to be sent in cross-origin requests, which could leak sensitive information like URLs or query parameters.
**Learning:** Security headers must be explicitly set in API middleware. While `X-Content-Type-Options`, `X-Frame-Options`, `Strict-Transport-Security`, `X-XSS-Protection`, and `Content-Security-Policy` were configured, `Referrer-Policy` was omitted.
**Prevention:** Always ensure a comprehensive suite of security headers is implemented, including `Referrer-Policy: no-referrer`, to prevent unintended data leakage via referer headers.
