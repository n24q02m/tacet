...
## 2025-02-21 - Fix missing security headers on 500 error responses
**Vulnerability:** Unhandled exceptions in FastAPI endpoints bypass custom HTTP middleware execution, preventing global security headers (e.g., CSP, X-Frame-Options) from being applied to the resulting 500 Internal Server Error responses.
**Learning:** In the Starlette/FastAPI execution stack, `ServerErrorMiddleware` catches exceptions bubbling up from inner layers and creates 500 responses directly, bypassing any downstream middleware logic. Custom HTTP middleware must wrap `call_next` in `try...except` to catch exceptions, format generic responses, and apply headers itself.
**Prevention:** Always implement generic exception handling globally or wrap routing/middleware execution properly so that response modifications (such as crucial security headers) remain intact on unhandled failure paths.
