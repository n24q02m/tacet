...
## 2025-02-21 - Added Global Exception Handler for Security Headers on 500 Responses
**Vulnerability:** Unhandled server exceptions (HTTP 500s) bypassed the custom HTTP middleware (`add_security_headers`) and were caught directly by Starlette's `ServerErrorMiddleware`. This stripped all previously configured security headers (like Content-Security-Policy, HSTS) from the 500 error response.
**Learning:** Custom HTTP middleware in FastAPI wraps standard endpoint execution but can be circumvented by deeper framework exception handlers. To enforce defense-in-depth across *all* responses (including generic unhandled faults), it is necessary to override global exception handling logic explicitly while allowing explicit HTTP exceptions (`HTTPException`) to pass through normally.
**Prevention:** Registered a global `@app.exception_handler(Exception)` that safely re-raises framework `HTTPExceptions` and manually injects required security headers (CSP, HSTS, X-Frame-Options, X-Content-Type-Options, X-XSS-Protection) into the `PlainTextResponse` for all generic faults, effectively overriding `ServerErrorMiddleware`'s default un-hardened response.
