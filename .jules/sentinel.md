...
## 2025-02-21 - Do not intercept generic exceptions in FastAPI to apply security headers
**Vulnerability:** Security headers are not applied to unhandled 500 errors because `ServerErrorMiddleware` catches exceptions before custom HTTP middleware, resulting in a plain text 500 response.
**Learning:** Adding an `@app.exception_handler(Exception)` that re-raises HTTP exceptions (`if isinstance(exc, HTTPException): raise exc`) to bypass this is considered a framework anti-pattern and breaks legitimate 4xx routing because raising inside the handler forces the ASGI server to drop the request and return a 500. Furthermore, FastAPI's default behavior already suppresses stack traces on 500 errors, so adding a handler solely for that is "security theater".
**Prevention:** Only apply security enhancements (like `Referrer-Policy: no-referrer`) to standard responses via standard middleware, and rely on the framework's native `ServerErrorMiddleware` for unhandled exceptions unless overriding it entirely via a custom ExceptionMiddleware.
