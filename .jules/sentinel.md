...
## 2025-02-14 - Prevent Denial of Service (DoS) via Unbounded Pydantic Inputs
**Vulnerability:** The FastAPI request models (`AskRequest`, `DistillRequest`, and `GraphIngestRequest`) previously accepted unbounded strings and lists. An attacker could exploit this by sending excessively large payloads, leading to significant memory consumption and CPU overhead during Pydantic validation, effectively causing a Denial of Service.
**Learning:** Pydantic's default `str` and `list` typings do not enforce any length limits. When defining models for FastAPI endpoints, it's crucial to explicitly bound these types, particularly for elements nested inside collections where parsing overhead multiplies.
**Prevention:** Always enforce strict length limits on input fields. Use `Field(..., max_length=X)` for top-level strings or collections. For strings nested inside lists or tuples, apply constraints using `typing.Annotated`, for example: `list[Annotated[str, Field(max_length=X)]]`.
