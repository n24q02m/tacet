...
## 2025-02-21 - [Security Headers on Error Responses]
**Vulnerability:** Unhandled exceptions in endpoints bypass the response phase of custom HTTP middleware, causing standard security headers to be absent from 500 error responses.
**Learning:** In Starlette/FastAPI, ServerErrorMiddleware wraps custom `@app.middleware('http')` functions. If an unhandled exception occurs, it is not caught by the custom middleware, skipping header injection and exposing a response without security headers.
**Prevention:** Wrap `await call_next(request)` in a `try...except Exception` block, explicitly re-raise `HTTPException`s (to preserve expected 4xx/5xx behavior), and manually return a secure 500 `JSONResponse` to apply headers before returning.
