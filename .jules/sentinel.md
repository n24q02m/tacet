...
## 2026-07-28 - Properly handle security headers on unhandled exceptions
**Vulnerability:** Unhandled exceptions that bubbled up to FastAPI's built-in `ServerErrorMiddleware` bypassed the custom HTTP middleware (`add_security_headers`), causing 500 error responses to be sent without essential security headers.
**Learning:** `ServerErrorMiddleware` is the outermost exception catcher in Starlette. Custom HTTP middlewares run inside of it, so an exception raised in an endpoint will skip the post-processing phase of the custom middleware. Catching generic `Exception` inside the middleware itself circumvents Starlette's safe handling.
**Prevention:** Use a global `@app.exception_handler(Exception)` that explicitly logs the error securely, formats a safe 500 response, applies the security headers manually via a helper function, and explicitly re-raises `HTTPException` (both FastAPI and Starlette variants) to preserve standard framework responses.
