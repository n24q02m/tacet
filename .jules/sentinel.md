...
## 2025-02-21 - Prevent required schema fields masking by Pydantic Field constraints
**Vulnerability:** A required Pydantic field (`answers`) in `DistillRequest` was defined using `Field(max_length=1000)` without specifying `...` (the Ellipsis) as the first argument. In Pydantic, using `Field` without `...` (or a default value) inadvertently makes the field optional and can assign `None` or cause validation to pass if missing, leading to unexpected application state and potential unhandled exceptions down the line.
**Learning:** When using `Field()` to add constraints (like `max_length`) to a field that must remain required, the first argument to `Field()` must explicitly be `...` (e.g. `Field(..., max_length=X)`). If this is omitted, Pydantic does not enforce the field as required.
**Prevention:** Always use `Field(..., ...)` when applying constraints to required properties on Pydantic request models.
