...
## 2025-02-21 - [Prevented Silently Swallowed Exceptions]
**Vulnerability:** The global `@app.exception_handler(Exception)` in `src/tacet/serve/server.py` returned a 500 without logging the exception, silently swallowing server-side tracebacks and preventing incident response.
**Learning:** Overriding FastAPI's default exception handlers bypasses `ServerErrorMiddleware`'s automatic exception logging. Custom handlers must explicitly log exceptions.
**Prevention:** Always explicitly log the exception `logging.error(..., exc_info=_exc)` inside a custom global exception handler before returning the secure response.
