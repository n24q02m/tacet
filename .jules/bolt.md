...
## 2025-02-21 - Python dict overhead in hot paths
**Learning:** In heavily used loops like the `RuleEngine._join` and `materialise` logic, allocating default empty objects in methods like `dict.setdefault(key, [])` or `dict.get(key, [])` creates significant garbage collection churn. Replacing these with simple `if/else` checks or checking `if candidates is not None:` after a plain `.get()` halves the execution time in dense logic programs.
**Action:** Always favor explicit membership checks `if key not in d: d[key] = [...]` over `.setdefault` or `.get(..., [])` when inside a recursive generator or heavily trafficked nested loop.
