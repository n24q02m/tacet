...
## 2024-07-06 - Optimize path-mining relation lookup in rule synthesis
**Learning:** Found the exact same performance bottleneck in `src/tacet/distill/distill.py` where `mine_rules` repeatedly rebuilds forward adjacency structures (`_adj(_directed(idx[r], inv))`) inside nested loops for rule synthesis over length-2 bodies. By pre-computing these adjacency cache maps once, the time taken for rule mining on a 100-entity graph dropped from 13.5s to 9.2s.
**Action:** Always pre-compute directed adjacency maps before evaluating length-2 (or greater) body groundings in symbolic miners.
