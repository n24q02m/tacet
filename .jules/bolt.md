...
## 2026-07-28 - Avoid redundant allocations and precalculate patterns in hot loops
**Learning:** Found a major bottleneck in `src/tacet/core/symbolic.py` within the `_join` method where `extend` was repetitively checking `_is_var` inside recursive loops, allocating default empty lists (`idx_subj.get(..., [])`) on every miss, and suffering overhead from function calls to `_unify`. This caused performance degradation in knowledge base materialization.
**Action:** When optimizing extreme recursive inner loops (like graph traversal or joining), pre-compute fixed patterns (`_is_var(x)`) outside the loop, inline hot path evaluations (`_unify`) manually to avoid function overhead, and use dictionary `get` without fallbacks (`get(x)`) and check for `None` to prevent object allocations on misses.
