...
## 2026-08-28 - Avoid allocations and function calls in recursive closures
**Learning:** In deeply recursive structural logic execution like `RuleEngine._join` in `src/tacet/core/symbolic.py`, making function calls like `_unify` inside loop bodies, passing generic data structures, or allocating empty structures via `.setdefault(key, [])` creates extreme execution overhead that accumulates rapidly as branches fan out.
**Action:** When a method is called frequently in nested loops, statically pre-calculate properties (like variable presence) into structures outside the loop. Inline small helper logic (like unification) directly into the hot recursive scope. Avoid dynamic default list allocations by doing a `.get` and exiting early if the lookup returns `None`. Defer dictionary copying until the exact point it's guaranteed to be used.
