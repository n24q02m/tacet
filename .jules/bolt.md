...
## 2026-08-01 - Avoid generic generator unrolling in FCA concepts enumeration
**Learning:** The FCA concept enumeration in `src/tacet/distill/fca.py` was previously using repeated generator unrolling `all()`, un-cached property lookups for `incidence` and `_attr_extents`, double-allocated set operations `(B - {k for k in B if k > m}) | {m}`, and heavy function call overheads with intermediate object allocations in `objects_of` and `attrs_of`.
**Action:** In extreme Python hot paths where polynomial iterations are made over structures, inline functions to avoid function call overhead and early-exit manually using `for` loops, reuse allocated `set` memory by mutating it with `intersection_update` over iterators instead of standard functional style map generators, and cache variables that invoke property access routines.
