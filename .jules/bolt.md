...
## 2026-07-28 - Inline _unify and precalculate static flags in _join
**Learning:** In deeply recursive matching loops like `_join`, repeatedly checking static properties (e.g., `_is_var`) and creating default empty lists (e.g., `get(key, [])`) causes significant constant overhead. Function call overhead to `_unify` for every fact candidate is also a major bottleneck.
**Action:** When working with deep backtracking or recursive matching logic, hoist static condition checks outside the loop, avoid creating default empty structures unnecessarily, and inline logic for extremely tight loops.
