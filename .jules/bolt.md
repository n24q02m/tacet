...
## 2026-07-28 - Optimize Datalog join streams by inlining _unify
**Learning:** In `src/tacet/core/symbolic.py`, calling `_unify` for every evaluated candidate inside the recursive `extend` loop of `RuleEngine._join` introduced heavy function call overhead. Additionally, `_unify` prematurely copied the binding dictionary even when new variable bindings might fail later or no bindings were needed.
**Action:** When optimizing extreme hot paths involving recursive pattern matching, inline the unification logic directly into the recursive loop to eliminate function call overhead. Defer expensive allocations like `dict.copy()` until all variable bindings are confirmed and actually needed.
