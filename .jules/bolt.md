...
## 2026-09-11 - Optimize FCA intent generation hot path
**Learning:** Found a performance bottleneck specific to this codebase's architecture in `src/tacet/distill/fca.py` where `_next_intent` repeatedly calls `objects_of` and `attrs_of`, causing significant function call overhead, set comprehensions and multiple frozenset/set allocations within an exponential loop.
**Action:** When working on extreme hot paths in Python (like Ganter's NextClosure in formal concept generation), inline function calls, avoid generator allocations (e.g., using `intersection_update` over `set(generator)`), and pre-cache repeated property access (like `_attr_extents` and `incidence`) outside the loops.
