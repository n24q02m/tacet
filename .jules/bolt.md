...
## 2026-09-17 - Optimize lattice edge extraction
**Learning:** In `src/tacet/distill/fca.py`, extracting direct hierarchical relationships (like Hasse diagram covers) using a topologically sorted concept lattice (`lattice_edges`) had an $O(N^3)$ bottleneck from iterating over all intermediate concepts to check if one is a superset. By maintaining a `covered_by` list of previously found direct covers, we avoid intermediate scans and check if a potential parent is a superset of any concept in `covered_by`, reducing complexity to $O(N^2 \times \text{width})$.
**Action:** When extracting direct hierarchical relationships in a topologically sorted concept lattice, maintain a `covered_by` list of previously found direct covers. Checking if a potential parent is a superset of any concept in `covered_by` avoids $O(N^3)$ intermediate scans.
