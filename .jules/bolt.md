...
## 2026-07-28 - Optimizing rule join matching via pattern unrolling
**Learning:** Found a performance bottleneck specific to Datalog evaluation hotpaths in `RuleEngine._join` inside `src/tacet/core/symbolic.py`. By precomputing pattern static shapes (`_is_var`) outside the recursive inner loop, substituting `dict.get(..., [])` with an explicit `is None` check to avoid repeated empty-list creations, and inlining the `_unify` dictionary copy logic entirely to dodge function call overhead, we reduced nested traversal times substantially. The recursive bench on fully connected transitive relations completed in ~0.69s instead of ~1.17s.
**Action:** When writing high-throughput recursive search combinations in Python, always try to inline operations instead of deferring to subroutines and delay allocations like `dict.copy()` until all constraints hold.
