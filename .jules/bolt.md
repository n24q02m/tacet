...
## 2025-02-21 - Python recursive Datalog matching hotpath
**Learning:** In highly recursive constraint solvers (like `RuleEngine._join`'s `extend`), function call overhead (`_unify`), generator comprehensions, and default intermediate list instantiations (`get(key, [])`) compound significantly.
**Action:** Always precalculate static configuration outside recursive functions, replace `get(key, [])` with `get(key)` and checking for `None`, and inline tiny utility functions (`_unify`) directly into hot recursive scopes to defer dict allocation until a definitive state change happens.
