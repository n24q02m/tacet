...
## 2026-08-15 - Optimize compliance rule mining
**Learning:** Found a major bottleneck in `src/tacet/distill/compliance_miner.py` where `mine_compliance_rules` reconstructed matched cases sets for every prefix during apriori generation via a `matches(pattern)` function. This caused repeated `O(N)` set intersections. Additionally, testing domination used a generator expression in `any()` and iterated over the entire `kept` list rather than grouped by target.
**Action:** When building combinations iteratively (apriori), cache the matched index set of the prefix to replace $O(N)$ repeated set intersections with a single `prefix & last_atom` step. Also use an early-exit loop combined with grouping structures instead of generator-based `any()` to avoid allocation overhead during nested combinatorial testing.
