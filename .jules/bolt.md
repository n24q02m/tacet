...
## 2026-07-27 - Inline _unify inside RuleEngine._join loop
**Learning:** The `RuleEngine._join` method iterates over candidate triples recursively extending bindings. The generic `_unify` call in this loop adds substantial overhead through function calls, repeated `_is_var` checks, and unconditional dictionary allocations early.
**Action:** Precompute variable checks on the body atoms, inline the unification rules directly into the `extend` loop to avoid function calls, and delay `dict.copy()` until all binding matches are validated to safely skip allocations.
