...
## 2026-08-01 - Optimize apriori pattern mining and fca iterations
**Learning:** Repeated O(N) set intersections and using generator expressions within any() in tight loops (like pattern dominance checks) causes significant overhead. Additionally, iterating over nodes to then find their edges when we can just iterate over edges directly is an O(N^2) trap.
**Action:** Cache intermediate set intersections when building combinations. Group items into dictionaries to reduce search space during hot path lookups. Replace any() generator expressions with explicit early-exit loops. Always iterate directly over the collection of interest (e.g. edges) instead of going through a parent object (nodes).
