...
## 2026-07-28 - Optimize Apriori generation and dominance pruning in compliance mining
**Learning:** In `src/tacet/distill/compliance_miner.py`, Apriori-style pattern generation recalculated matches from scratch for each level, and most-general-first dominance pruning evaluated a generator expression over the entire kept list for every candidate. This resulted in O(N^2) scaling behavior and massive generator overhead on larger datasets.
**Action:** Cache the matched case indices for prefix patterns to replace repeated O(N) set intersections with a single O(N) intersection per candidate. Group target data by case ID to avoid redundant object lookups. Also, use explicit early-exit loops over grouped structures instead of generator expressions in `any()` to avoid allocation overhead during hot-path dominance testing.
