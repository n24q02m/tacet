...
## 2025-02-21 - Pre-compute static checks in Datalog join
**Learning:** `RuleEngine._join` recursively called `extend` which performed redundant dictionary default allocations (`get(..., [])`) and checks like `_is_var(s)` for each atom in the rule body. By pre-computing variable checks before the recursion and returning early on `None` from `get`, the join processing time for dense graphs reduced from 1.44s to 1.37s.
**Action:** When writing recursive backtracking or Datalog join algorithms, lift static property checks (like variable detection) out of the recursive function and use default-less dictionary lookups to avoid allocating unneeded empty lists on cache misses.
