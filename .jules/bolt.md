...
## 2026-07-28 - Optimize FCA concept enumeration
**Learning:** In extreme Python hot paths (such as FCA concept enumeration in `src/tacet/distill/fca.py`), replacing generic generator unrolling functions like `any()` and `all()` with explicit early-exit loops, and avoiding double allocation during set constructions (e.g., using direct comprehensions and `.add()`) eliminates significant function call overhead and premature allocations.
**Action:** Replace generic generator unrolling functions like `any()` and `all()` with explicit early-exit loops, and avoid double allocation during set constructions in hot paths.
