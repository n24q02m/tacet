...
## 2026-07-28 - Inlined fixed-tuple `_unify` reduces constant overhead
**Learning:** Found a major bottleneck in `RuleEngine._join` where generating permutations delegates graph facts verification via a heavily recursive loop invoking a separate `_unify` function. Invoking this distinct method across deeply nested data searches caused massive constant function call overhead and early repetitive dictionary allocations.
**Action:** When repeatedly calling a relatively small verifier like `_unify` inside a highly iterative structure like an `Iterator` generator or combinatorical recursive tree, consider inline unrolling. Precompute invariant arguments and push evaluations immediately onto their values and branch states rather than making a function call and delegating overhead.
