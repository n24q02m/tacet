...
## 2025-02-21 - Symbolic Pattern Matching Optimization Bug
**Learning:** Inlining core logic functions (like `_unify`) for micro-optimizations in hot loops can easily introduce regressions if abstraction layers are broken. Specifically, hardcoding `.startswith("?")` instead of using the central `_is_var()` definition causes bugs if the engine's definition of a variable changes.
**Action:** Always prefer using provided helper functions (`_is_var()`) over hardcoding logic, even during aggressive inlining. The overhead of a simple helper function is negligible compared to the maintenance cost and risk of introducing subtle bugs by breaking abstractions.
