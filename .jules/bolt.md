...
## 2026-07-31 - Fast gradient scattering with flattened bincount
**Learning:** When accumulating gradients in KGE models over large batches where memory thresholds prevent one-hot matmul (`len(idx) * n > 40_000_000`), `np.add.at` is excessively slow. Replacing it with `np.bincount` over flattened 1D indices `(idx[:, None] * c + np.arange(c)).ravel()` yields a ~3x speedup on large batches with equivalent memory footprint.
**Action:** Always prefer flattened `np.bincount` over `np.add.at` when aggregating dense 2D matrices across large batches.
