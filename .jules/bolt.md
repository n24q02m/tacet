...
## 2025-02-21 - Optimize RuleEngine._join by avoiding redundant loop checks and dictionary allocations
**Learning:** In `tacet/core/symbolic.py`, `RuleEngine._join` is highly recursive. Pre-computing static properties about the variables and avoiding empty `list()` creations using `dict.get(key, [])` significantly speed up combinatorial evaluation by reducing O(N) allocation overhead per evaluation branch.
**Action:** Pre-compute tuple attributes if it can be hoisted out of a recursive loop and use `dict.get()` when dictionary items are large, ensuring list checks fall back gracefully `if candidates is not None:`.
