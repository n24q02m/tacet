...
## 2026-07-30 - Inline closures and avoid set generator allocations in `_next_intent`
**Learning:** Found a major performance bottleneck in `src/tacet/distill/fca.py`'s `_next_intent` loop. Unpacking `objects_of` and `attrs_of` locally into explicit iterators relying on `intersection_update` prevents huge overhead compared to passing `frozenset` instances globally and using standard generator expressions.
**Action:** When working on nested tight mathematical computations like Ganter's NextClosure algorithm, inline core function access explicitly utilizing mutable structures (`intersection_update` on sets) to avoid O(N) intermediary generator allocations and dict lookups.
