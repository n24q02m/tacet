...
## 2025-02-22 - Optimize _join by inlining _unify and preventing dict allocation
**Learning:** Found a performance bottleneck specific to this codebase's architecture in `src/tacet/core/symbolic.py` where `_join` recursively called `_unify` and repeatedly copied binding dictionaries. By precomputing properties and inlining the logic inside `extend`, we bypassed function call overhead, prevented premature dict allocation, and prevented instantiating empty lists for misses on dictionary gets.
**Action:** When working on extreme hot paths in Python where variables can be pre-calculated in the outer loop block, inline the core evaluation loop directly into recursive paths to reduce the constant overhead.
