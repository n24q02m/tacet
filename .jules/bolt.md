...
## 2026-07-28 - Optimize Datalog join evaluation
**Learning:** Found a performance bottleneck in `src/tacet/core/symbolic.py` during rule engine materialisation where `RuleEngine._join` was spending significant time in hot path recursion calling generic `_unify` logic and allocating temporary dictionaries. By inlining `_unify`, pre-computing static pattern properties like `_is_var`, and deferring `dict.copy()` until a valid binding is found, materialisation time can be heavily optimized without changing evaluation order.
**Action:** When working on extreme hot paths in Python recursive loops, consider inlining hot-path functions, pre-computing static invariants outside the loop, and aggressively delaying object allocations (like dictionary copies) until they are strictly necessary.
