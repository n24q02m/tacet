...
## 2026-07-28 - Inline `_unify` into Datalog join generator
**Learning:** In `src/tacet/core/symbolic.py`, evaluating rule bodies over Multi-relational graphs generated significant function call overhead by calling `_unify` for every checked fact.
**Action:** When working with Python's recursive generators in tight loops (e.g. Datalog rule evaluation), manually inline small functions like unification directly into the loop body and pre-parse rules to avoid expensive runtime checks (`_is_var`) and delayed state copying, yielding a roughly 3x speedup.
