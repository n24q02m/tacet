...
## 2026-09-17 - Unrolled iteration and removed allocations in FCA concept enumeration
**Learning:** In the extreme hot path of `_next_intent` during FCA lattice construction, standard Python generator expressions (e.g. `all()`), functional combinations like `set(intersection(…))`, and building full intermediate `frozenset` sets via multiple function calls to `objects_of` and `attrs_of` create massive Python runtime overhead due to high object churn and function dispatch on a tight nested loop.
**Action:** When calculating set intersections in algorithmic hot paths (such as formal concept generation), explicitly unroll generator checks into early-exit for-loops, use direct `intersection_update` to avoid temporary set creation, and inline critical calculations to minimize function call overheads.
