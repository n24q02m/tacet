...
## 2026-07-28 - Optimize Hasse diagram covers in FCA lattice
**Learning:** Found a performance bottleneck in `src/tacet/distill/fca.py` where `lattice_edges` checked for intermediate concepts between a child and parent by scanning all concepts of intermediate size, resulting in an O(N^3) traversal over concepts. By tracking already-found direct parents for each child in a `covered_by` list, and checking if any candidate parent is a superset of an already-found parent, intermediate scans were eliminated, reducing a 36-second check on ~7500 concepts down to 8 seconds.
**Action:** When extracting direct hierarchical relationships (like Hasse diagram covers) in a topologically sorted concept lattice, maintain a list of previously found direct covers to check against rather than explicitly iterating over all intermediate nodes.
