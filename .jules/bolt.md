...
## 2026-07-27 - Optimize FCA concept enumeration
**Learning:** Found extreme Python overhead in `src/tacet/distill/fca.py` during concept enumeration. Using generic generator expressions in `any()`/`all()` and making repeated calls to `objects_of` and `attrs_of` inside the hot `_next_intent` loop resulted in double memory allocations and function call overhead.
**Action:** In extreme hot paths with known loop counts, explicitly unpack variables, use explicit early-exit loops, use `.intersection_update()` for set logic instead of generic generator unrolling, and inline functions while caching properties (`_attr_extents` and `incidence`) outside the loops.
