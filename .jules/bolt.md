...
## 2026-07-27 - Fast lattice covers via dynamic `covered_by` cache
**Learning:** Found an O(N^3) bottleneck in `src/tacet/distill/fca.py`'s `lattice_edges` where checking if a parent directly covers a child required an inner loop over all intermediate-sized concepts. Since we iterate parents in increasing size, any valid intermediate concept would have already been found as a direct cover. Checking if the potential parent is a superset of any concept in `covered_by` reduces complexity to O(N^2 * width), yielding nearly a 10x speedup for moderately dense lattices (e.g. 4.6s down to 0.5s).
**Action:** When extracting direct hierarchical relationships (like Hasse diagram covers) in a topologically sorted concept lattice, maintain a list of previously found direct covers to avoid O(N^3) intermediate scans.
