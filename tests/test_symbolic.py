...
class TestRuleEngine(unittest.TestCase):
    ...
    def test_join_streams_bindings_instead_of_materialising_them(self) -> None:
        ...
        class AccessCountingList(list):
            def __init__(self, *args, counter_dict=None):
                super().__init__(*args)
                self.counter_dict = counter_dict or {"n": 0}

            def __iter__(self):
                for item in super().__iter__():
                    self.counter_dict["n"] += 1
                    yield item
        ...
        for fact in facts:
            h, r, t = fact
            if r not in idx_all:
                idx_all[r] = AccessCountingList(counter_dict=accesses)
            idx_all[r].append(fact)
            if (r, h) not in idx_subj:
                idx_subj[(r, h)] = AccessCountingList(counter_dict=accesses)
            idx_subj[(r, h)].append(fact)
            if (r, t) not in idx_obj:
                idx_obj[(r, t)] = AccessCountingList(counter_dict=accesses)
        ...
        # One candidate evaluated per body atom to reach the first row. Materialising the
        # join first would cost ~2n (600) before yielding anything.
        self.assertLessEqual(accesses["n"], 2 * len(body))
