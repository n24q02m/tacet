...
class TestAgent(unittest.TestCase):
    ...
    def test_planner_with_derived_triples(self) -> None:
        ...
        # Goal: Alice is at a place that has a path to Lyon.
        # If she flies to Paris, she's at a place that has a path to Lyon.
        goal = Goal((("alice", "at", "?somewhere"), ("?somewhere", "path", "lyon")))

        plan = planner.plan(g, goal)
        self.assertIsNotNone(plan)
        self.assertEqual(len(plan.actions), 1)
        self.assertEqual(dict(plan.actions[0].bindings)["?to"], "paris")
